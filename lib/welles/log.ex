defmodule Welles.Log do
  ...
  def create_log(log_name) do
    case Log.Manager.start_link(name: supervisor_name(log_name)) do
      {:ok, _pid} -> {:ok, :created}
  ...
  In the next example we attempt to add an event to a log that doesn't exist.
      ...
      iex> Welles.Log.add_event("my_event", "my_log")
      :not_found
  ...
  def add_event(event, log_name) do
    case Log.Writer.add_event(event, writer_name(log_name)) do
      :ok -> :added
      {:error, :not_found} -> :not_found
    end
  end

  @spec supervisor_name(String.t()) :: String.t()
  defp supervisor_name(log_name), do: "#{log_name}_supervisor"
  @spec writer_name(String.t()) :: String.t()
  defp writer_name(log_name), do: "#{log_name}_writer"
end
